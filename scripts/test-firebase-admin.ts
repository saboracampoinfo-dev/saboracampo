/**
 * Script de Prueba - Firebase Admin Credentials
 * 
 * Este script verifica que las credenciales de Firebase Admin estén configuradas correctamente
 * sin necesidad de hacer un registro completo.
 * 
 * Uso:
 * 1. Asegúrate de que tu .env.local tenga las credenciales actualizadas
 * 2. Ejecuta: npm run test:firebase-admin
 * 
 * O directamente:
...
try {
  ...
  console.error('❌ No se pudo cargar .env.local:', error);
  process.exit(1);
}
...
async function testFirebaseAdmin() {
  ...
  try {
    // Importar dinámicamente para evitar problemas si las credenciales faltan
