/**
 * Script alternativo para insertar sucursales usando una API de Node.js nativa
 * Ejecutar con: node scripts/seed-sucursales-http.js
 */
...
async function seedSucursales() {
  try {
    ...
    const existingData = await makeRequest('GET', '/api/sucursales');
    ...
    for (let i = 0; i < sucursalesEjemplo.length; i++) {
      ...
      const result = await makeRequest('POST', '/api/sucursales', sucursal);
    ...
    const finalData = await makeRequest('GET', '/api/sucursales');
