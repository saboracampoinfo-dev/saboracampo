...
export default function BarcodeScanner({ onScan, disabled = false, placeholder = "Escanea código de barras" }: BarcodeScannerProps) {
  ...
  const TIEMPO_ESPERA_ESCANEO = 1500; // 1.5 segundos entre escaneos del mismo código
    ...
    setTimeout(async () => {
      try {
        ...
        await scanner.start(
          ...
          (decodedText) => {
            // Verificar si es el mismo código escaneado recientemente
            const ahora = Date.now();
            const ultimoEscaneo = ultimoEscaneoRef.current;
            
            if (ultimoEscaneo && 
                ultimoEscaneo.codigo === decodedText && 
                ahora - ultimoEscaneo.timestamp < TIEMPO_ESPERA_ESCANEO) {
