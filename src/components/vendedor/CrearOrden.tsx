...
export default function CrearOrden() {
  ...
  const crearNuevaOrden = async () => {
    setLoading(true);
    try {
      // Obtener sucursal desde localStorage
      const sucursalGuardada = localStorage.getItem('sucursalActiva');
      const sucursalNombreGuardado = localStorage.getItem('sucursalActivaNombre');

      const requestBody: any = { action: 'crear' };
      
      // Si hay sucursal guardada, enviarla
      if (sucursalGuardada && sucursalNombreGuardado) {
  ...
  const agregarProductoPorId = async (productoId: string) => {
    ...
    console.log('🔵 [CrearOrden] Intentando agregar producto por ID');
    console.log('🔵 productoId:', productoId);
    console.log('🔵 ordenId:', orden._id);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-xl text-primary font-semibold animate-pulse">Creando orden...</div>
      </div>
    );
  }
  ...
  if (!orden) {
    return (
      <div className="bg-error-light/10 border border-error-light text-error-dark rounded-lg p-2 md:p-4">
        Error al cargar la orden. Por favor, recarga la página.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-linear-to-r from-primary to-primary-700 text-white rounded-lg p-2 md:p-6 shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-4">
          <div>
            <h2 className="text-xl md:text-3xl font-bold">Nueva Orden de Venta</h2>
            <div className="mt-2 space-y-1">
              ...
              {orden.sucursal && orden.sucursal.nombre ? (
                <p className="text-sm opacity-90 bg-white/20 px-2 py-1 rounded inline-block mt-1">
                  📍 Sucursal: {orden.sucursal.nombre}
                </p>
              ) : (
                <p className="text-sm opacity-90 bg-error-light/30 px-2 py-1 rounded inline-block mt-1">
        ...
        <div className="bg-white dark:bg-dark-700 rounded-lg p-2 md:p-6 shadow-md border border-dark-200 dark:border-dark-600">
          ...
          <form onSubmit={handleCodigoBarrasSubmit} className="flex gap-3">
            <input
              ...
              disabled={procesando}
              className="flex-1 px-2 md:px-4 py-3 border border-dark-300 dark:border-dark-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary dark:bg-dark-800 dark:text-light-500 text-lg"
            />
            <button
              type="submit"
      ...
      <div className="bg-white dark:bg-dark-700 rounded-lg p-2 md:p-6 shadow-md border border-dark-200 dark:border-dark-600">
        ...
        {orden.productos.length === 0 ? (
          <div className="text-center py-8 text-dark-500 dark:text-dark-400">
            No hay productos agregados. Escanea un código de barras para comenzar.
          </div>
        ) : (
          <div className="space-y-3">
            {orden.productos.map((producto) => (
              <div
                ...
                className="flex items-center gap-4 p-4 bg-dark-50 dark:bg-dark-800 rounded-lg border border-dark-200 dark:border-dark-600"
              >
                {producto.imagen && (
                  <img
                    src={producto.imagen}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <button
          ...
          className="cursor-pointer flex-1 bg-success-light hover:bg-green-800 disabled:bg-dark-300 disabled:cursor-not-allowed text-white px-6 py-4 rounded-lg text-lg font-bold transition-all duration-300"
        >
          ✓ Cerrar Orden y Enviar a Caja
        </button>
        <button
          onClick={cancelarOrden}
          className="cursor-pointer flex-1 bg-error-light hover:bg-red-600 dark:hover:bg-red-600 text-white px-6 py-4 rounded-lg text-lg font-bold transition-all duration-300"
        >
          ✕ Cancelar Orden
        </button>
      </div>
      ...
      {/* Instrucciones */}
      <div className="bg-primary-100 dark:bg-dark-800 border border-primary rounded-lg p-4">
        <h4 className="font-bold text-primary mb-2">📋 Instrucciones:</h4>
        <ul className="space-y-1 text-sm text-dark-700 dark:text-dark-300">
          <li>• <strong>Escanear:</strong> Usa el lector de código de barras para agregar productos rápidamente</li>
          <li>• <strong>Buscar:</strong> Escribe el nombre del producto para buscarlo manualmente</li>
          <li>• Si un producto ya está agregado, se incrementará su cantidad en 1</li>
          <li>• Usa los botones + / - para ajustar cantidades manualmente</li>
          <li>• Al cerrar la orden, pasará a &quot;Pendiente de Cobro&quot; para que un cajero la procese</li>
          <li>• Puedes ver todas tus órdenes en la pestaña &quot;Mis Órdenes&quot;</li>
        </ul>
      </div>
    </div>
  );
